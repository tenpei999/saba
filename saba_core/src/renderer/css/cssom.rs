...
impl CssParser {
  ...
  fn consume_list_of_rules(&mut self) -> Vec<QualifieldRule> {
    //空のベクタを作成する
    ...
    loop {
      ...
      match token {
        ...
        CssToken::AtKeyword(_keyword) => {
          let _rule = self.consume_qualifield_rule();
          // しかし本書のブラウザでは@から始まるルールはサポートしないので、無視する
        }
        _ => {
          //1つのルールを解釈し、ベクタに追加する
          let rule = self.consume_qualifield_rule();
          match rule {
            Some(r) => rules.push(r),
            None => return rules
          }
  ...
  fn consume_qualifield_rule(&mut self) -> Option<QualifieldRule> {
    let mut rule = QualifieldRule::new();
  ...
  fn consume_selector(&mut self) -> Selector {
    ...
    match token {
      ...
      CssToken::Ident(ident) => {
        // a:hoverのようなせレクタはタイプセレクタとして扱うため、
      ...
      CssToken::AtKeyword(_keyword) => {
        // @から始まるルールを無視するため、宣言ブロックの開始直前までトークンを進める
        while self.t.peek() != Some(&CssToken::OpenCurly) {
          self.t.next();
        }
  ...
  fn consume_ident(&mut self) -> String {
    ...
    match token {
      ...
      _ => {
        panic!("Paese error: {:?} is an unexpected token.", token);
      }
    }
  }

  /// https://www.w3.org/TR/css-syntax-3/#consume-component-calue
  fn consume_component_value(&mut self) -> ComponentValue {
    self.t
        .next()
        .expect("sould have a token in consume_component_value")
  }
}
...
impl StyleSheet {
  ...
  pub fn set_rules(&mut self, rules: Vec<QualifieldRule>) {
    self.rules = rules;
  }
}

#[derive(Debug, Clone, PartialEq)]
pub struct QualifieldRule {
  /// https://www.w3.org/TR/selectors-4/#typedef-selector-list
  pub selector: Selector,
  /// https://www.w3.org/TR/selectors-3/#parse-a-list-of-declarations
...
mod tests {
  ...
  fn test_multiple_rulesr() {
    ...
    let mut rule2 = QualifieldRule::new();

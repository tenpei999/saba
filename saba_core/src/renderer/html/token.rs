...
impl HtmlTokeneizer {
  ...
  fn take_last_token(&mut self) -> Option<HtmlToken> {
    assert!(self.latest_token.is_some());
...
impl Iterator for HtmlTokeneizer {
  type Item = HtmlToken;
    ...
    loop {
      ...
      match self.state {
        ...
        State::TemporaryBuffer => {
          ...
          .chars()
          .nth(0)
          .expect("self.buf should have at last 1char");
          self.buf.remove(0);
          return Some(HtmlToken::Char(c));
        }

        // _=> {} //全ての状態を網羅したため、この行は削除する。
      }
...
mod tests {
  ...
  fn test_empty() {
    let html = "".to_string();
    let mut tokenizer = HtmlTokeneizer::new(html);
